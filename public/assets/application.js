...
(function() {
  $(function() {
    return $("#ad_show #thumbs .thumb").hover(function() {
      var index;
      ...
      return $("#big_images .big_image:nth-child(" + (index + 1) + ")").show();
    });
  });
...
(function() {
  jQuery(function() {
    var car_models, escaped_car_models, make, options;
    ...
    return $('#make_id').change(function() {
      $('#car_model_id').html('');
      make = $("option:selected", this).text();
      escaped_car_models = make.replace(/([ #;&,.+*~\':"!^$[\]()=>|\/@])/g, '\\$1');
      options = $(car_models).filter("optgroup[label='" + escaped_car_models + "']").html();
      options = ["<option value=\"\"></option>"] + options;
      return $('#car_model_id').html(options);
    });
  });
